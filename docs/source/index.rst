...
*Note: Both sides of the range of dates are inclusive, and the unique identifier
must be valid in combination with the source.*
...
*Note: Both sides of the range of dates are inclusive, the unique
...
the optimisation problem might be infeasible.*
...
optimal allocation will sum to 100. A future version of the
package will address this issue.**

*Note: The weights are returned as integers, and those integers represent
percentage values (i.e., 25 is equivalent to an allocation of 25%).*
